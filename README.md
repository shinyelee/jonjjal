...
- 좋은 짤(meme)을 만들 수 있는 그림판 프로젝트입니다.
- [데모 사이트 바로가기][데모]
...
- 슬라이더(input type="range")를 이용해 선 굵기를 조절합니다. 아래 텍스트가 현재 브러시의 굵기를 알려줍니다.
...
- 지우기 버튼을 클릭하면 맨 마지막에 그린 선부터 역순으로 한 획을 지우며, 리셋 버튼을 클릭하면 캔버스가 초기화 됩니다.
...
- 폰트 크기, 폰트 유형을 선택할 수 있습니다. 텍스트 입력란에 원하는 문구를 입력 후 캔버스 위에서 더블클릭을 하면 해당 위치에 텍스트가 삽입됩니다.
